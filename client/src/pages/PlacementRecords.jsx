...
import { Search, Plus, X, Trash2, Eye, Briefcase, Loader2 } from 'lucide-react'
      ...
      {activeTab === 'Matcher' ? (
        <div className='grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8'>
          <div className='glass-panel p-5 rounded-2xl border border-gray-100 flex items-center gap-4 bg-white/60'>
            ...
            <div>
              <p className='text-sm font-semibold text-gray-500'>Total in Scope</p>
              <p className='text-xl font-bold text-gray-800'>Master List</p>
            </div>
          </div>
          ...
          <div className='glass-panel p-5 rounded-2xl border border-green-100 flex items-center gap-4 bg-green-50/30'>
            ...
            <div>
              ...
              <p className='text-xl font-bold text-green-800'>Uploaded</p>
            </div>
          </div>
          ...
          <div className='glass-panel p-5 rounded-2xl border border-red-100 flex items-center gap-4 bg-red-50/30'>
            ...
            <div>
              <p className='text-sm font-semibold text-red-600'>Defaulters</p>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8'>
          <div className='glass-panel p-5 rounded-2xl border border-gray-100 flex items-center gap-4 bg-white/60'>
            ...
            <div>
              ...
              <p className='text-xl font-bold text-gray-800'>Outcomes</p>
            </div>
          </div>
          ...
          <div className='glass-panel p-5 rounded-2xl border border-indigo-100 flex items-center gap-4 bg-indigo-50/30'>
            <div className='w-12 h-12 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-lg'>
              {offerLetters.filter((r) => r.type === 'Job' || !r.type).length}
            </div>
            <div>
              <p className='text-sm font-semibold text-indigo-600'>Placed</p>
              <p className='text-xl font-bold text-indigo-800'>Job Offers</p>
            </div>
          </div>
          ...
          <div className='glass-panel p-5 rounded-2xl border border-purple-100 flex items-center gap-4 bg-purple-50/30'>
            <div className='w-12 h-12 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center font-bold text-lg'>
              {offerLetters.filter((r) => r.type === 'Higher Studies').length}
            </div>
            <div>
                        ...
                        <button
                          onClick={() => {
                            if (!record.letterUrl || record.letterUrl === '#') {
                              ...
                              window.open(record.letterUrl, '_blank', 'noopener,noreferrer')
                            }
                          }}
                    ...
                    <tr key={req._id || index} className='hover:bg-blue-50/20 transition-colors'>
                      ...
                      <td className='py-4 px-6 text-center'>
                        <div className='flex justify-center items-center gap-2'>
                          <button
                            onClick={() => handleApproveNoDues(req._id)}
                            disabled={isProcessing}
                            className='px-3 py-1.5 bg-green-50 text-green-600 hover:bg-green-500 hover:text-white rounded-lg font-bold text-xs transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-green-50 disabled:hover:text-green-600 inline-flex items-center gap-1.5'
                          >
                          ...
                          <button
                            onClick={() => handleRejectNoDues(req._id)}
                            disabled={isProcessing}
                            className='px-3 py-1.5 bg-red-50 text-red-600 hover:bg-red-500 hover:text-white rounded-lg font-bold text-xs transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-red-50 disabled:hover:text-red-600 inline-flex items-center gap-1.5'
                          >
                            {isProcessing ? <Loader2 size={14} className='animate-spin' /> : null}
                    ...
                    <td className='py-3 px-6'>
                      <div className='flex flex-col'>
                        <span className='text-gray-700 font-semibold text-xs'>
                          {record.degree || 'B.Tech'} - {record.branch}
                        </span>
        ...
        {showAddModal && (
          <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
            ...
            <motion.div
              ...
              className='glass-panel w-full max-w-lg rounded-3xl shadow-2xl relative z-10 p-6 overflow-hidden max-h-[90vh] overflow-y-auto'
            >
              <div className='flex justify-between items-center mb-6'>
                ...
                <button
                  ...
                  className='p-2 bg-gray-50 text-gray-400 hover:text-gray-600 rounded-full disabled:opacity-50'
                >
                  <X size={20} />
                </button>
              </div>
              ...
              <form onSubmit={handleAddPlacement} className='space-y-4 text-sm'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                  <div>
                    ...
                    <select
                      required
                      className='glass-input w-full p-3 bg-white/50 appearance-none text-sm'
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                  <div>
                    <label className='block text-gray-700 font-semibold mb-1'>
                      {newPlacement.type === 'Higher Studies' ? 'University / Institute' : 'Company'}
                    </label>
                    <input
                      required
                      type='text'
                  ...
                  <div>
                    <label className='block text-gray-700 font-semibold mb-1'>
                      {newPlacement.type === 'Higher Studies' ? 'Program / Degree' : 'Package'}
                    </label>
                    <input
                      required
                      type='text'
                ...
                <div>
                  ...
                  <input
                    ...
                    className='glass-input w-full p-3 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100'
                    onChange={(e) =>
                      setNewPlacement({
                        ...newPlacement,
                        letterPdf: e.target.files?.[0] || null
                      })
                    }
                  />
                ...
                <button
                  type='submit'
                  disabled={isAddingPlacement}
                  className='btn-primary w-full py-3 mt-4 rounded-xl shadow-lg disabled:opacity-70 disabled:cursor-not-allowed inline-flex items-center justify-center gap-2'
                >
